...
class Menu(object):
	...
	def __init__(self):
		...
		pygame.display.set_caption("Labyrinthe des IUT")
	...
	# Foncion initialisant les questions s'affichant dans les options
	...
	def actualiserQuestion(self):
		...
		for i in range(len(self.questions)):
			...
			fenetre.append(Rect((self.largeur-taille)//2,y,taille,self.questions[i][1].height*3//2))
			self.parametre[i].set_pos((self.largeur-taille)//2+self.questions[i][1].height//4,y+self.questions[i][1].height//4)
		...
		if i == 1:
			if self.parametre[i].value.isdigit():
				if self.parametre[0].value.isdigit():
					return 0<= int(self.parametre[i].value) < 5-int(self.parametre[0].value)
		if i == 2:
			if self.parametre[i].value.isdigit():
				...
				if self.parametre[1].value.isdigit():
					if self.parametre[2].value.isdigit():
						if self.parametre[3].value.isdigit():
							return 0 < int(self.parametre[3].value) <= int(self.parametre[2].value)//(int(self.parametre[0].value)+int(self.parametre[1].value))
		return False
	...
	def initMenu(self):
		...
		for i in range(len(listeMenu)):
			if i == 0:
				...
				listeMenu[i][1].y = listeMenu[i-1][1].y + listeMenu[i-1][1].height + self.espace
		self.menu=listeMenu
	...
	def demarrer(self):
		...
		while continuer:
			...
			if self.select == 0:
				...
				if ok:
					g = LabyrintheGraphique(Labyrinthe(int(self.parametre[0].value),int(self.parametre[2].value),int(self.parametre[3].value),int(self.parametre[1].value)))
					g.demarrer()
					self.initMenu()
					self.select=0
				else:
