...
class PlayerToInsert:
    def __init__(self, name : str, 
        ...
        if player_id is None:
            ...
            assert is_human == False, "player_id must not be provided if is_human is True"
    ...
    def get_or_create_human_player(self, session) -> int:
        ...
        player = session.query(Player).filter(Player.name == self.name, Player.is_human == True).first()
...
class SetOfRulesToInsert:
    def __init__(self, rules_ids : list[int], set_of_rules_id : int = None):
        if len(rules_ids) == 0:
            assert set_of_rules_id is not None, "set_of_rules_id must be provided if rules_ids is empty"
    ...
    def get_or_create_set_of_rules(self, session) -> int:
        ...
        existing_set_of_rules = session.query(SetOfRules).join(IsRuleOf).filter(IsRuleOf.rule_id.in_(self.rules_ids)).group_by(SetOfRules.set_of_rules_id).having(func.count(SetOfRules.set_of_rules_id) == len(self.rules_ids)).first()
        ...
        for rule_id in self.rules_ids:
            is_rule_of = IsRuleOf(set_of_rules_id=new_set_of_rules.set_of_rules_id, rule_id=rule_id)
...
class ParticipantToInsert:
    ...
    def create_participant(self, session) -> int:
        participant = session.query(Participant).filter(Participant.game_id == self.game_id, Participant.player_id == self.player_id).first()
        if participant is None:
            participant = Participant(game_id=self.game_id, player_id=self.player_id, turn_order=self.turn_order, score=self.score, nb_moves=self.nb_moves, is_winner=self.is_winner)
            session.add(participant)
            print(f"Participant ajouté : {participant.player_id} pour le jeu {participant.game_id}")
        else:
...
def store_final_game_data(
    ...
    with Session() as session:
        ...
        game = Game(set_of_rules_id=set_of_rules_id, played_at=datetime.now(), nb_participants=len(players))
        session.add(game)
